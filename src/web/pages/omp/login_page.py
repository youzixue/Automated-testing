...
class OmpLoginPage:
    ...
    ERROR_MESSAGE_SELECTOR = "//p[contains(@class, 'el-message__content')]" 
    ERROR_DIALOG_SELECTOR = ".el-message-box"
    ERROR_DIALOG_CONTENT_SELECTOR = ".el-message-box__message p"
    SUCCESS_URL_PATTERN = r".*/workbench.*"
    ...
    async def check_login_success(self, timeout: float = 10) -> bool:
        """检查是否成功登录（通过URL跳转判断）。"""
        try:
            await self.driver.page.wait_for_url(re.compile(self.SUCCESS_URL_PATTERN), timeout=timeout * 1000)
            self.logger.info(f"登录成功，已跳转到预期页面: {self.driver.page.url}")
            return True
        except Exception as e:
            self.logger.warning(f"登录后未跳转到预期页面: {e}")
            return False

    async def check_login_error(self) -> Optional[str]:
        """检查页面上是否存在登录错误提示。"""
        self.logger.debug("检查登录错误提示...")
        try:
            # 再次确认移除 has_element 调用中的 timeout 参数
            if await self.driver.has_element(self.ERROR_MESSAGE_SELECTOR):
                error_el = await self.driver.get_element(self.ERROR_MESSAGE_SELECTOR)
                if error_el:
                    error_text = await error_el.text_content()
                    if error_text:
                        self.logger.warning(f"检测到错误消息: {error_text.strip()}")
                        return error_text.strip()
            
            # 再次确认移除 has_element 调用中的 timeout 参数
            if await self.driver.has_element(self.ERROR_DIALOG_SELECTOR):
                 error_content_el = await self.driver.get_element(self.ERROR_DIALOG_CONTENT_SELECTOR)
                 if error_content_el:
                     error_text = await error_content_el.text_content()
                     if error_text:
                         self.logger.warning(f"检测到错误弹窗: {error_text.strip()}")
                         return error_text.strip()

            self.logger.debug("未检测到明确的登录错误提示")
            return None
        except Exception as e:
            self.logger.warning(f"检查登录错误提示异常: {e}", exc_info=False) 
            return None

                            ...
                            if (params.RUN_APP_RELATED_TESTS) {
                                ...
                                if (primaryDeviceSerial && !primaryDeviceSerial.isEmpty()) {
                                    ...
                                    docker run --rm --name adb-check-main-${BUILD_NUMBER} \\
                                      ...
                                      --privileged \\
                                      --network host \\
                                      ${env.DOCKER_IMAGE} sh "${checkScriptPathInContainer}" "${primaryDeviceSerial}" || (echo "错误: 容器内检查主设备 ${primaryDeviceSerial} 的脚本执行失败!" && exit 1) \
                                    ...
                                    if (useTwoDevices) { 
                                        ...
                                        docker run --rm --name adb-check-sec-${BUILD_NUMBER} \\
                                          ...
                                          --privileged \\
                                          --network host \\
                                          ${env.DOCKER_IMAGE} sh "${checkScriptPathInContainer}" "${secondaryDeviceSerial}" || (echo "错误: 容器内检查次设备 ${secondaryDeviceSerial} 的脚本执行失败!" && exit 1) \
